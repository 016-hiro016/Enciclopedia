...
class Category(db.Model):
    ...
    entries=db.relationship("Entry",back_populates="category",lazy=True,cascade="all, delete-orphan")
...
def create_app():
    ...
    app.config.update(SQLALCHEMY_DATABASE_URI=url,SQLALCHEMY_TRACK_MODIFICATIONS=False,SECRET_KEY=os.getenv("SECRET_KEY","omnisiah-secret"))
    ...
    def search():
        ...
        p=f"%{q}%"; items=Entry.query.join(Category).filter(or_(Entry.title.ilike(p),Entry.description.ilike(p),Entry.body.ilike(p),Entry.tags.ilike(p),Category.name.ilike(p))).limit(80).all()
